...
async def extract_pokemon_model(raw_pokemon: dict, session: aiohttp.ClientSession, lang: str) -> Pokemon:
    pokemon_abilities = raw_pokemon.get('abilities', [])
    pokemon_abilities_urls = list(
        map(lambda pokemon_dict: pokemon_dict.get('ability', {}).get('url'), pokemon_abilities))
    abilities_details = await asyncio.gather(*[fetch(session, url) for url in pokemon_abilities_urls],
